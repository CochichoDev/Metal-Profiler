...
#include "api/tcpsimple2.h"
#include "api/t32nettcp.h"
...
static int t32NetTcpInit(char *message)
{
	...
	tcpMsg.msg_data[ 0] = (uint8_t)((T32_NETTCP_VERSION >> 0 ) & 0xFF);
	tcpMsg.msg_data[ 1] = (uint8_t)((T32_NETTCP_VERSION >> 8 ) & 0xFF);
	tcpMsg.msg_data[ 2] = (uint8_t)((T32_NETTCP_VERSION >> 16) & 0xFF);
	tcpMsg.msg_data[ 3] = (uint8_t)((T32_NETTCP_VERSION >> 24) & 0xFF);

	tcpMsg.msg_data[ 4] = (uint8_t)((T32_NETTCP_INTFTYPE_RCL >> 0) & 0xFF);
	tcpMsg.msg_data[ 5] = (uint8_t)((T32_NETTCP_INTFTYPE_RCL >> 8) & 0xFF);

	tcpMsg.msg_data[ 8] = (uint8_t)((T32_NETTCP_RCL_VERSION >>  0) & 0xFF);
	tcpMsg.msg_data[ 9] = (uint8_t)((T32_NETTCP_RCL_VERSION >>  8) & 0xFF);
	tcpMsg.msg_data[10] = (uint8_t)((T32_NETTCP_RCL_VERSION >> 16) & 0xFF);
	tcpMsg.msg_data[11] = (uint8_t)((T32_NETTCP_RCL_VERSION >> 24) & 0xFF);
...
#if T32_NETTCP_VER_MINOR(T32_NETTCP_VERSION) > 0
...
#if T32_NETTCP_VER_MINOR(T32_NETTCP_RCL_VERSION) > 0
	// If server does not support at least our minor version exit
	if (T32_NETTCP_VER_MINOR(srvRclVersion) < T32_NETTCP_VER_MINOR(T32_NETTCP_RCL_VERSION))
		goto errExit;
#endif
...
static const struct T32InternalLineDriver gT32NetTcpDrv = {
...
const struct T32InternalLineDriver *gT32InternalLineDriverNetTcp = &gT32NetTcpDrv;
