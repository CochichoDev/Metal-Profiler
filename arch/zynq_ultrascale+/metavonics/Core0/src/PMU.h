...
/*static inline void enable_glob_evt_counters() {
	register u64 value;
	 __asm__ __volatile("MRS %0, PMCR_EL0"  : "=r" (value));
	 __asm__ __volatile("MSR PMCR_EL0, %0"  :: "r" (value | PMCR_ENABLE_MASK));
}

static inline void enable_evt_counter(u8 ctr) {
	register u64 value;
	 __asm__ __volatile("MRS %0, PMCNTENSET_EL0"  : "=r" (value));
	 __asm__ __volatile("MSR PMCNTENSET_EL0, %0"  : "r" (value | (1 << ctr)));
}*/
...
static inline void no_allocate_threshold_L1(u32 mode) {
	register u64 value = 0;
 ...
 *
 */
static inline void no_allocate_threshold_L2(u32 mode) {
	register u64 value = 0;
