...
#if defined (mmu4)
    block_size = BLOCK_SIZE_4KB;
    section = Addr / block_size;
    ptr = &MMUTableL3 + section;
...
void Xil_SetTlbAttributesRange(UINTPTR AddrStart, UINTPTR AddrEnd, u64 attrib)
{
	INTPTR ptr;
	INTPTR section;
	u64 block_size;
    ...
    for (ptr = AddrStart ; ptr < AddrEnd ; ptr += block_size) {
        Xil_SetTlbAttributes(ptr, attrib);
    }
}
